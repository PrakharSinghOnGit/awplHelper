...
async function level(page: any, name: string, id: string, pass: string) {
  ...
  const minIndex = Math.min(
    TargetSAOs.findIndex((targetSAO) => sao < targetSAO),
    TargetSGOs.findIndex((targetSGO) => sgo < targetSGO)
  );
  const pendingSAO = Math.round(
    (minIndex === -1 ? 0 : TargetSAOs[minIndex] - sao) < 0
      ? 0
      : TargetSAOs[minIndex] - sao
  );
  const pendingSGO = Math.round(
    (minIndex === -1 ? 0 : TargetSGOs[minIndex] - sgo) < 0
      ? 0
      : TargetSGOs[minIndex] - sgo
  );
